...
BEGIN
#check if zone exists in rr table, if it is null return nonExistingZone,
#otherwise store zone id into local @zoneId variable
SET @zoneId = getZoneIdFromRecord(record);
# check if user from the api key matches the owner of the updated zone in SOA table
SET @rootDomainOwner = (SELECT owner FROM mydns.soa WHERE owner = getUserIdFromApiKey(apikey)
						AND id = @zoneId);                        
#check if record exists in rr table, if does save id into local @recordId variable
SET @recordId = (SELECT id FROM mydns.rr WHERE name = record);

if (@zoneId AND @rootDomainOwner AND @recordId ) THEN
	UPDATE mydns.rr SET data = newData WHERE id = @recordId;
	SELECT "OK";
else
	SELECT @zoneId,@rootDomainOwner,@recordId;
END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2020-03-17  3:00:29
